...
export function useBroadcastSending(): UseBroadcastSendingReturn {
  ...
  async function resolveAudience(audience: AudienceConfig): Promise<Contact[]> {
    ...
    if (audience.type === 'all') {
      ...
      if (tagError)
        throw new Error(`Falha ao buscar etiquetas dos contatos: ${tagError.message}`);
  ...
  async function upsertCsvContacts(
    ...
    const { data: existing, error: lookupErr } = await supabase
      ...
      .in('phone', phones);
    if (lookupErr) {
      throw new Error(`Falha ao localizar contatos do CSV: ${lookupErr.message}`);
    }
  ...
  async function createAndSendBroadcast(payload: BroadcastPayload): Promise<string> {
    ...
    try {
      ...
      if (broadcastError || !broadcast) {
        throw new Error(
          `Falha ao criar campanha: ${broadcastError?.message ?? 'erro desconhecido'}`,
        );
      }
      ...
      if (recipientsFetchError || !recipients) {
        throw new Error('Falha ao buscar destinatários da campanha');
      }
      ...
      for (let i = 0; i < recipients.length; i += SEND_BATCH_SIZE) {
        ...
        try {
          ...
          if (!res.ok) {
            throw new Error(data.error || 'Falha na solicitação da API de campanhas');
          }
